...
content_nmea = str(lines[0]).split('and')

def getGPSData():
	#Declaracion variables
	fileSensorReadings = os.getcwd()+'/python_module/code/'+'sensor_readings_output.csv'
	countCheck = 0
	countParse = 0
	tim = []
	lat = []
	...
	return matrix
def getSpeedTests():
	fileSensorInsights = os.getcwd()+'/python_module/code/'+'sensor_insights_output.csv'
	countCheck = 0
	countParse = 0
	#Declaracion variables
	tim = []
	...
	num_sprints=0
	x=datetime.strptime("05:00:00", "%H:%M:%S")
	#Se abre archivo
	pd.options.mode.chained_assignment = None  # default='warn'
	for line in content_nmea:
		content = line.split(',')
	...
	for i,line in enumerate(matrix["Time"]):
	   ...
	   t_graficar=str(matrix["Time"][i]-x)
	   tt.append(t_graficar)
	tiempoGrafico=np.delete(tt, (0), axis=0)
...
#transformar a minutos
	...
	max_speed=round(matrix_datos_finales["vmps"][0],3)
	for i,line in enumerate(matrix_datos_finales["type"]):
		if i ==0:
			...
			if matrix_datos_finales["type"][i] == sprints:
				...
				if matrix_datos_finales["vmps"][i] >= max_speed:
					max_speed=round(matrix_datos_finales["vmps"][i],3)
...
def executeAlgorithm():
    ...
    jsonfile = open(os.getcwd()+'\\python_module\\code\\'+outputFile, 'w')

    #Reading insights
    csvFileSensorReadings = open(os.getcwd()+'\\python_module\\code\\'+fileSensorReadings, 'r')
    numLinesSensorReadings = sum(1 for line in open(os.getcwd()+'\\python_module\\code\\'+fileSensorReadings))
    ...
    #Reading readings
    csvFileSensorInsights = open(os.getcwd()+'\\python_module\\code\\'+fileSensorInsights, 'r')
    numLinesSensorInsights= sum(1 for line in open(os.getcwd()+'\\python_module\\code\\'+fileSensorInsights))


    fieldnames = ("no","accel","accel_type","accel_zone","delta_height","distance","lat_1","lat_2","lng_1","lng_2","time","type","vkph","vmps")
...
def get_results():
    executeAlgorithm()
    with open(os.getcwd()+'\\python_module\\code\\'+outputFile) as data_file:
